...
class StudentRepository(BaseRepository[Student]):
    ...
    async def get_list(
        ...
        if first_names:
            stmt = stmt.where(Student.first_name.in_(first_names))

        if last_names:
            stmt = stmt.where(Student.first_name.in_(last_names))

        if second_names:
            stmt = stmt.where(Student.first_name.in_(second_names))

        if start_date:
            stmt = stmt.where(Student.birth_day >= start_date)

        if end_date:
            stmt = stmt.where(Student.birth_day <= datetime.combine(end_date, time.max))
